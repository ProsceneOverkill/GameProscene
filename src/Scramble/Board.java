...
import static Scramble.Scramble.scene;
...
public class Board {
    ...
    public Board(int size, PImage img, PApplet parent) {
        this.parent = parent;
        this.size = size;
        this.img = img;
        order();
    ...
    static PImage pimg = null; // current patch image
    public void order() {
        clear();
            ...
            for (int x = 0; x < size; x++) // for each column
                if (number < size * size) {
                    if (img != null) {
                        ...
                        pimg = parent.createImage(img.width/ size, img.height / size, RGB); // a new image for the current patch
    ...
    public void scramble() {
        ...
        for (int i = 0; i < size * size * 1000; i++) // iterate far enough to ensure we get a scrambled board
            movePatch(patches[(int) parent.random(0, size)][(int) parent.random(0, size)]); // move a patch randomly
        moves = 0;
    }
